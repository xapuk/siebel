(() => {
    ...
    const html = `<div title="Inspect Workspace"><span id = "${func}Help" style = "display:none">${help}</span><input placeholder = "<${placeholder}>" type="text" id = "${func}" list="${func}History" autocomplete="off"><ul id="${func}List"></ul><p id = "${func}Msg"></p><datalist id = "${func}History"></datalist><style>.${func} input {width: 100%!Important;}#${func}::placeholder {color: lightslategrey;}#${func}List {margin-left: 15px;}#${func}Help i {font-size: 0.9rem;}.${func} li {list-style-type: disc;margin-left: 30px;}</style></div>`;
